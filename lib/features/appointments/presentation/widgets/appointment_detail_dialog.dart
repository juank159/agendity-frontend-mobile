...
//   const AppointmentDetailsDialog({Key? key, required this.appointment, a})
//       : super(key: key);
...
class AppointmentDetailsDialog extends StatelessWidget {
  ...
  Future<bool> _isOwner() async {
    try {
      // Usamos FlutterSecureStorage directamente o a través del UserInfoController
      // dependiendo de cómo prefieras implementarlo
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Sección del Profesional - Solo se muestra para Owner
                    FutureBuilder<bool>(
                      future: _isOwner(),
                      builder: (context, ownerSnapshot) {
                        final bool isOwner = ownerSnapshot.data ?? false;

                        // Solo mostramos la información del profesional si el usuario es Owner
                        if (isOwner &&
                          ...
                          // Si no es Owner, no mostramos esta sección
                    ...
                    FutureBuilder<bool>(
                      future: _isOwner(),
                      builder: (context, snapshot) {
                        final bool isOwner = snapshot.data ?? false;

                        return Column(
                          crossAxisAlignment: CrossAxisAlignment.stretch,
                          children: [
                            // Botón de cerrar siempre visible para todos los roles
                            ElevatedButton(
                              onPressed: () => Navigator.pop(context),
                              child: const Text('Cerrar'),
                              style: ElevatedButton.styleFrom(
                            ...
                            // Mostrar botones adicionales solo si el usuario es Owner y la cita no está pagada
                            if (isOwner && !isPaid) ...[
                              const SizedBox(height: 8),
                              ...
                              ElevatedButton.icon(
                                onPressed: () async {
                                  ...
                                  // Mostrar el diálogo de edición y esperar el resultado
                                  final result =
                                      await EditAppointmentDialog.show(
                                    context,
                                    appointment,
                                    appointmentsController,
                                  );
